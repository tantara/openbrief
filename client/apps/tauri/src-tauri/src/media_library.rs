...
fn save_media_library_snapshot_to_connection(
    ...
    for (video_id, summary) in object_field(snapshot, "summariesByVideoId") {
        ...
        let artifact_path = summary_artifact_path(video_id, &summary_id);
...
fn summary_artifact_path(video_id: &str, summary_id: &str) -> String {
    format!(
        "videos/{}/summary/{}.md",
        sanitize_path_segment(video_id),
    ...
    object_field(snapshot, "summariesByVideoId")
        ...
        .map(|(_, summary)| {
            summary
                ...
                .and_then(Value::as_str)
                .map(str::to_string)
                .unwrap_or_else(|| summary_artifact_path(video_id, value_id(summary)))
        })
        .collect()
}
...
mod tests {
    ...
    fn saves_and_loads_snapshot_through_sql_tables() {
        ...
        assert_eq!(
            ...
            "videos/video-1/summary/summary-video-1.md"
        );
        ...
        assert_eq!(
            fs::read_to_string(library.join("videos/video-1/summary/summary-video-1.md")).unwrap(),
            "# Summary"
        );
        assert!(
    ...
    fn migrates_legacy_thumbnail_and_transcript_paths_into_video_bundle() {
        ...
        assert_eq!(
            fs::read_to_string(library.join("videos/video-1/thumbnail/poster.jpg")).unwrap(),
            "image"
        );
        assert_eq!(
